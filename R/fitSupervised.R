fitSupervised <- function(hmm, modelData) {
  ...
  emission_params <- data.frame(
    mu = rep(NA, nrow(dataGLM_expanded)),
    nb_size = rep(NA, nrow(dataGLM_expanded)),
  ...
  transMat <- lapply(modelData_fit_id, function(x) {
    table(
      factor(x$state[1:(nrow(x) - 1)], levels = c(0, 1)),
  ...
  list(
    hmm = hmm, hmm_expectation = hmm_expectation, model = dataGLM_expanded,
